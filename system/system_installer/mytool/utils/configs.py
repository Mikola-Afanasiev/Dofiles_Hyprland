...
class ConfigManager:
    ...
    def update_configs(self):
        ...
        if replacing_configs.returncode != 0:
            log(
                replacing_configs.stderr, level="error", code=6)
    ...
    def sync_configs(self):
        ...
        if result.returncode == 0:
            log("Files are identical.", level="success")
        else:
            log("Files aren't identical", level="warning")
            log("Starting updating", level="info")
            # updating confings
            ...
            if updating_configs_check:
                ...
                log(
                    "Updating configs, something went wrong", level="error", code=4
                )
    ...
    def zsh_shell_changing(self):
        ...
        if shell == "/usr/bin/zsh":
            log(
                "You have already had zsh as shell", level="success")
        else:
            result = subprocess.run(["bash", "-c", "chsh -s $(which zsh)"])
            if result.returncode != 0:
                log("Error with zsh", level="error", code=5)
