    ...
    for category_name, file_list in config_list.items():
        ...
        for file_name in file_list:
            ...
            if os.path.isfile(exist_file):
                log.log(f"{file_name} exist", level="info")
            else:
                log.log(f"{file_name} doesn't exist", level="warning")
                helpers_missing = False

    # Checking for all helpers
    if helpers_missing:
        log.log("All helpers are here, starting working...", level="success")
    else:
        log.log("Helpers aren't all here", level="error", code=2)
